        ...
        <div className="px-6 pb-6">
          <Tabs value={tab} onValueChange={(v) => { setTab(v as "login" | "signup"); setErr(null); }}>
            ...
            <TabsContent value="login" className="pt-4 space-y-3">
              <FloatingInput id="li-email" label="Email" type="email" value={li.email} onChange={(v) => setLi({ ...li, email: v })} autoComplete="off" />
              <FloatingInput id="li-pw" label="Password" type="password" value={li.password} onChange={(v) => setLi({ ...li, password: v })} autoComplete="new-password" />
            ...
            <TabsContent value="signup" className="pt-4 space-y-3">
              <FloatingInput id="su-name" label="Full name" value={su.name} onChange={(v) => setSu({ ...su, name: v })} autoComplete="off" />
              <FloatingInput id="su-email" label="Email" type="email" value={su.email} onChange={(v) => setSu({ ...su, email: v })} autoComplete="off" />
              <FloatingInput id="su-phone" label="Phone" value={su.phone} onChange={(v) => setSu({ ...su, phone: v })} autoComplete="off" />
              <div className="grid grid-cols-2 gap-3">
                <FloatingInput id="su-pw" label="Password" type="password" value={su.password} onChange={(v) => setSu({ ...su, password: v })} autoComplete="new-password" />
                <FloatingInput id="su-cf" label="Confirm" type="password" value={su.confirm} onChange={(v) => setSu({ ...su, confirm: v })} autoComplete="new-password" />
              </div>
...
function FloatingInput({ id, label, value, onChange, type = "text", autoComplete }: { id: string; label: string; value: string; onChange: (v: string) => void; type?: string; autoComplete?: string }) {
  return (
    <div className="relative">
      <Input id={id} type={type} value={value} onChange={(e) => onChange(e.target.value)} className="h-12 pt-4 peer" placeholder=" " autoComplete={autoComplete} />
