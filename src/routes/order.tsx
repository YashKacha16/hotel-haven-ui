...
import { menu, BRAND, type MenuItem } from "@/lib/data";
...
export const Route = createFileRoute("/order")({
  head: () => ({
    meta: [
      { title: `Order to your room — ${BRAND.name}` },
...
const HAS_ACTIVE_BOOKING = true; // dummy in-house guest
const ROOM_NUM = "204";
...
function OrderPage() {
  ...
  const [oid] = useState(() => "ORD-" + Math.floor(10000 + Math.random()*90000));

  const items = useMemo(() => Object.entries(menu).flatMap(([cat, arr]) => arr.map((i) => ({ ...i, cat }))), []);
  ...
  const place = () => requireAuth("Place your order", () => {
    setStatus(0);
    toast.success("Order placed!", { description: `${oid} · ETA 30 min` });
              ...
              <div className="flex-1 overflow-y-auto space-y-3 py-4">
                ...
                {items.filter((i) => cart[i.name]).map((i) => (
                  <div key={i.name} className="flex gap-3">
                    <img src={i.img} className="h-16 w-16 rounded-md object-cover" />
                    ...
                    <div className="text-sm">₹{(cart[i.name] * i.price).toLocaleString()}</div>
                  </div>
                ))}
              </div>
              <div className="border-t border-border pt-4 space-y-3">
                <Textarea placeholder="Special instructions…" value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} />
                <div className="flex justify-between font-serif text-lg"><span>Subtotal</span><span className="text-gold">₹{subtotal.toLocaleString()}</span></div>
                ...
                <Button className="w-full bg-gold text-gold-foreground hover:bg-gold/90 gold-glow" disabled={count === 0} onClick={place}>Place Order</Button>
              </div>
            </SheetContent>
          </Sheet>
        </div>

        <Tabs defaultValue={Object.keys(menu)[0]}>
          <TabsList className="flex flex-wrap h-auto bg-cream w-fit">{Object.keys(menu).map((c) => <TabsTrigger key={c} value={c}>{c}</TabsTrigger>)}</TabsList>
          {Object.entries(menu).map(([cat, arr]) => (
            <TabsContent key={cat} value={cat} className="mt-8">
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {arr.map((d: MenuItem, i) => (
                  <Reveal key={d.name} delay={i * 50}>
                    <Card className="overflow-hidden border-0 shadow-sm hover:shadow-lg transition py-0 gap-0">
                      ...
                      <div className="p-5">
                        <div className="flex items-start justify-between gap-3"><h3 className="font-serif text-lg">{d.name}</h3><span className="font-serif text-lg text-gold">₹{d.price}</span></div>
                        <p className="mt-2 text-sm text-muted-foreground line-clamp-2">{d.desc}</p>
                        <div className="mt-3 flex gap-1.5 flex-wrap">{d.tags.map((t) => <Badge key={t} variant="secondary" className="text-[10px]">{t}</Badge>)}</div>
